...
def check_file_history():
    ...
    # whole file history
        ...
        if len(data) < 4:
            print("[red]Could not retrieve file history.[/red]")
            print("[yellow]The file may not be tracked by Git yet.[/yellow]")
            return

        commit, author, date, message = [x.strip() for x in data]
